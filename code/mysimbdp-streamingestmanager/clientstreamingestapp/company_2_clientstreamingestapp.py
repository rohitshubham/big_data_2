...
logging.basicConfig(filename= "../../logs/company2/streamingestapp.log" , filemode="a", level= logging.INFO, format=custom_logging_format)
...
#topic-name = "big_data_" + truncate(sha256("company_2"), 5)
...
def connectAndInsertRow(valueToBeStored):
    current_collection = __connectToDatabase("company_2_stream")
...
def reporting_data():
    ...
    if time.time() - last_time_sent > 6:
        data = ["client2", avg_processing_time, total_messages-last_reported_messages, total_ingestion_data_size]
        send_mqtt_requests(data)
        logging.info(f"Reporting: Sent MQTT report to auto scaling code. The data was {str(data)}") 
        total_ingestion_data_size = 0
        avg_processing_time = 0
...
def on_message(client, userdata, msg):
    try:
        ...
        logging.info(f"Received message for client_2. start the ingestion of data into the database.")
        start = time.time()
